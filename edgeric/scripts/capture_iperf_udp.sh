#!/bin/bash
# sudo ./capture_iperf_udp.sh ../telemetry-runs/2026-03-18/run8 -d
...
if [ -z "$OUTPUT_PATH" ]; then
    ...
    echo "Example: $0 ../telemetry-runs/2026-03-18/run5"
    echo "         $0 ../telemetry-runs/2026-03-18/run5 --dump"
    echo ""
    echo "Options:"
...
Experiment: UDP iPerf Flooding Test with EdgeRIC Metrics Collection
Date: $(date)
Duration: ${DURATION}s
tcpdump: ${DUMP_STATUS}

Components:
- iPerf3 UDP: sudo ip netns exec ${UE_NAMESPACE} iperf3 -c ${IPERF_SERVER} -p ${IPERF_PORT} -u -b 0M -i ${IPERF_INTERVAL} -t ${DURATION} -R
...
Output Files:
- iperf_udp.txt: iPerf3 UDP flooding output from ${UE_NAMESPACE} namespace
- metrics_udp.json: EdgeRIC metrics collected during the UDP test
${DUMP_FILES}

Test Configuration:
- UE namespace: ${UE_NAMESPACE}
- iPerf server: ${IPERF_SERVER}:${IPERF_PORT}
- Bandwidth: unlimited (0M = max rate)
...
sudo "$SCRIPT_DIR/../venv/bin/python" "$SCRIPT_DIR/../collector.py" --json --output "$OUTPUT_PATH/metrics_udp.json" &
COLLECTOR_PID=$!

sleep 2

echo "Starting iPerf3 UDP flooding test..."
sudo ip netns exec "$UE_NAMESPACE" iperf3 -c "$IPERF_SERVER" -p "$IPERF_PORT" -u -b 0M -i "$IPERF_INTERVAL" -t "$DURATION" -R > "$OUTPUT_PATH/iperf_udp.txt" 2>&1 &
IPERF_PID=$!
